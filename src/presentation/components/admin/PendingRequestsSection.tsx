            ...
            return (
              <div key={req.id} className="border-[2px] sm:border-[3px] border-black p-2 sm:p-4 bg-white flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  {req.note && <p className="text-xs sm:text-sm mt-1 opacity-60 truncate">📝 {req.note}</p>}
                </div>
                <div className="flex gap-1.5 sm:gap-2 shrink-0">
                  <button onClick={() => handleApprove(req.id)} disabled={approvingId === req.id}
                    className="px-2 sm:px-4 py-1.5 sm:py-2 bg-emerald-500 text-white font-bold uppercase text-[10px] sm:text-xs border-[2px] border-emerald-700 hover:bg-emerald-600 transition-colors disabled:opacity-40">
                    {approvingId === req.id ? '...' : '✅ อนุมัติ'}
                  </button>
                  <button onClick={() => onReject(req.id)}
                    className="px-2 sm:px-4 py-1.5 sm:py-2 bg-red-500 text-white font-bold uppercase text-[10px] sm:text-xs border-[2px] border-red-700 hover:bg-red-600 transition-colors">
                    ❌ ปฏิเสธ
                  </button>
                </div>
              </div>
            ...
            return (
              <div key={req.id} className="border-4 border-black p-3 sm:p-4 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 hover:shadow-[4px_4px_0_0_rgba(0,0,0,1)] transition-all bg-white">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {req.note && <p className="text-sm mt-1 opacity-60 truncate">📝 {req.note}</p>}
                </div>
                <div className="flex gap-2 shrink-0">
                  <button onClick={() => handleApprove(req.id)} disabled={approvingId === req.id}
                    className="px-3 sm:px-4 py-2 bg-[#39FF14] text-black font-bold uppercase text-xs border-2 border-black hover:bg-[#00FFFF] transition-colors disabled:opacity-40 shadow-[2px_2px_0_0_rgba(0,0,0,1)]">
                    {approvingId === req.id ? '...' : '✅ APPROVE'}
                  </button>
                  <button onClick={() => onReject(req.id)}
                    className="px-3 sm:px-4 py-2 bg-red-500 text-white font-bold uppercase text-xs border-2 border-black hover:bg-red-600 transition-colors shadow-[2px_2px_0_0_rgba(0,0,0,1)]">
                    ❌ REJECT
                  </button>
                </div>
              </div>
          ...
          return (
            <div key={req.id} className="bg-surface rounded-xl border border-border p-2.5 sm:p-3 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
              <div className="flex-1 min-w-0">
                ...
                {req.note && <p className="text-xs text-muted mt-0.5 truncate">📝 {req.note}</p>}
              </div>
              <div className="flex gap-1.5 shrink-0">
                <button onClick={() => handleApprove(req.id)} disabled={approvingId === req.id}
                  className="px-2.5 sm:px-3 py-1.5 bg-emerald-500 text-white text-[10px] sm:text-xs font-bold rounded-lg hover:bg-emerald-600 transition-colors disabled:opacity-40">
                  {approvingId === req.id ? '...' : '✅ อนุมัติ'}
                </button>
                <button onClick={() => onReject(req.id)}
                  className="px-2.5 sm:px-3 py-1.5 bg-red-500 text-white text-[10px] sm:text-xs font-bold rounded-lg hover:bg-red-600 transition-colors">
                  ❌ ปฏิเสธ
                </button>
              </div>
            </div>
