    ...
    return (
      <div className="fixed inset-0 z-[999] flex flex-col items-center justify-center p-4 font-mono"
        ...
        <div className="relative z-30 w-full max-w-md text-center">
          ...
          <h1 className="text-4xl font-black text-[#39FF14] tracking-widest uppercase mb-3 drop-shadow-[0_0_10px_#39FF14]">สำเร็จ_</h1>
          <p className="text-[#00FFFF] font-bold uppercase text-xs tracking-widest mb-10">ขอคิวเรียบร้อย</p>
          <div className="flex flex-col sm:flex-row gap-6 items-center justify-center mb-10">
            ...
            <div className="border-[4px] border-[#FF00FF] bg-black p-6 shadow-[8px_8px_0_0_rgba(255,0,255,0.5)] flex-1 w-full">
              <div className="text-[10px] font-black uppercase tracking-[0.3em] text-[#00FFFF] mb-4">รหัสเช็คคิว =</div>
            ...
            {qrCodeUrl && (
              <div className="border-[4px] border-[#00FFFF] bg-white p-4 shadow-[8px_8px_0_0_rgba(0,255,255,0.5)] shrink-0">
                ...
                <div className="text-[10px] font-black text-black uppercase tracking-widest mt-2">สแกนเช็คคิว_</div>
              </div>
            )}
          </div>

          <p className="text-xs font-bold uppercase tracking-widest text-[#FF00FF] mb-10">
            กลับหน้าหลักอัตโนมัติใน {countdown} วินาที_
          </p>
          ...
          <button onClick={handleDone} className="inline-flex items-center gap-2 px-8 py-4 border-[4px] border-[#00FFFF] bg-black text-[#00FFFF] font-black uppercase tracking-widest hover:bg-[#00FFFF] hover:text-black transition-colors text-sm shadow-[4px_4px_0_0_rgba(0,255,255,0.5)]">
            <ArrowLeft className="w-5 h-5" /> กลับหน้าหลัก_
          </button>
        </div>
      </div>
    );
      ...
      <header className="flex items-center justify-between px-4 sm:px-8 py-3 bg-[#00FFFF] border-b-[4px] border-black shadow-[0_4px_0_0_rgba(0,0,0,1)] z-10 shrink-0">
        <div className="flex items-center gap-3">
          <Link href="/display" className="flex items-center gap-1.5 p-2 border-[3px] border-black bg-white hover:bg-[#FF00FF] hover:text-white transition-colors shadow-[2px_2px_0_0_rgba(0,0,0,1)] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none">
            <ArrowLeft className="w-5 h-5" strokeWidth={3} /><span className="hidden sm:inline text-xs font-black uppercase tracking-widest">กลับ_</span>
          </Link>
          <Ticket className="w-5 h-5 text-[#FF00FF]" strokeWidth={3} />
          <h1 className="text-lg font-black uppercase tracking-widest" style={{ WebkitTextStroke: '0.5px black' }}>รับคิว_</h1>
        </div>
      </header>
        ...
        <div className="w-full max-w-lg relative z-10">
          ...
          <form onSubmit={handleSubmit}>
            {currentStep === 'info' && (
              <div className="space-y-6">
                <div className="text-center mb-8">
                  <h2 className="text-3xl font-black tracking-widest uppercase text-[#FF00FF]" style={{ WebkitTextStroke: '1px black' }}>กรอกข้อมูล_</h2>
                  <p className="text-xs font-black uppercase tracking-widest mt-2 text-gray-500">// กรอกข้อมูลของท่านด้านล่าง</p>
                </div>
                <div>
                  ...
                  <div className="relative">
                    <input type="text" value={customerName} onChange={(e) => setCustomerName(e.target.value)} autoFocus
                      className="w-full px-4 py-3.5 pr-10 border-[3px] border-black bg-white placeholder:text-gray-300 focus:outline-none focus:shadow-[4px_4px_0_0_rgba(255,0,255,1)] transition-all text-base font-black" placeholder="กรอกชื่อของคุณ" />
                    {customerName && (
                      <button type="button" onClick={clearCustomerName} className="absolute right-3 top-1/2 -translate-y-1/2 px-2 py-1 text-[10px] font-black uppercase tracking-widest text-[#FF00FF] bg-black border-2 border-[#00FFFF] hover:bg-[#00FFFF] hover:text-black hover:border-black transition-colors" title="เคลียร์">CLEAR</button>
                    )}
                  </div>
                ...
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-widest mb-2 text-[#FF00FF]">$NOTE <span className="text-gray-300">(ไม่บังคับ)</span></label>
                  <div className="relative">
                    <input type="text" value={note} onChange={(e) => setNote(e.target.value)}
                      className="w-full px-4 py-3.5 pr-10 border-[3px] border-black bg-white placeholder:text-gray-300 focus:outline-none focus:shadow-[4px_4px_0_0_rgba(255,0,255,1)] transition-all text-base font-black" placeholder="ข้อมูลเพิ่มเติม..." />
                    {note && (
                      <button type="button" onClick={clearNote} className="absolute right-3 top-1/2 -translate-y-1/2 px-2 py-1 text-[10px] font-black uppercase tracking-widest text-[#FF00FF] bg-black border-2 border-[#00FFFF] hover:bg-[#00FFFF] hover:text-black hover:border-black transition-colors" title="เคลียร์">CLEAR</button>
                    )}
                  </div>
                  <div className="flex flex-wrap gap-2 mt-3">
                    {presets.notes.map((p) => <button key={p} type="button" onClick={() => appendNote(p)} className="px-3 py-1.5 text-[10px] border-2 border-black text-gray-500 hover:bg-[#00FFFF] hover:text-black transition-all font-black uppercase shadow-[2px_2px_0_0_rgba(0,0,0,1)] active:shadow-none active:translate-x-0.5 active:translate-y-0.5">+ {p}</button>)}
                  </div>
                </div>
                <button type="button" disabled={!canGoNext} onClick={() => setCurrentStep('verify')}
                  ...
                  ถัดไป_ <ArrowRight className="w-5 h-5" strokeWidth={3} />
                </button>
              </div>
            )}
            ...
            {currentStep === 'verify' && (
              <div className="space-y-6">
                <div className="text-center mb-8">
                  <h2 className="text-3xl font-black tracking-widest uppercase text-[#00FFFF]" style={{ WebkitTextStroke: '1px black' }}>ยืนยันตัวตน_</h2>
                  <p className="text-xs font-black uppercase tracking-widest mt-2 text-gray-500">// ตอบคำถามเพื่อยืนยัน</p>
                </div>
                <div className="border-[4px] border-[#00FFFF] bg-white p-6 sm:p-8 shadow-[8px_8px_0_0_rgba(0,255,255,0.5)]">
                  <div className="flex items-center gap-2 mb-6 text-[#FF00FF] text-[10px] font-black uppercase tracking-widest"><Lock className="w-4 h-4" strokeWidth={3} /> ป้องกันบอท_</div>
                  {challenge ? (
                  ...
                  ) : <div className="text-[#FF00FF] animate-pulse text-center py-8 font-black uppercase text-sm">กำลังโหลด_</div>}
                </div>
                <div className="flex gap-3 mt-4">
                  ...
                  <button type="button" disabled={!canGoNext} onClick={() => setCurrentStep('preview')} className="flex-1 py-4 border-[3px] border-black bg-[#39FF14] text-black font-black uppercase tracking-widest hover:bg-[#00FFFF] transition-all disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-[10px] shadow-[2px_2px_0_0_rgba(0,0,0,1)] active:shadow-none active:translate-x-0.5 active:translate-y-0.5">ถัดไป_ <ArrowRight className="w-5 h-5" strokeWidth={3} /></button>
                </div>
              </div>
            )}
            ...
            {currentStep === 'preview' && (
              <div className="space-y-6">
                <div className="text-center mb-8">
                  ...
                  <p className="text-xs font-black uppercase tracking-widest mt-2 text-gray-500">// ตรวจสอบข้อมูลและยืนยัน</p>
                </div>
                <div className="border-[4px] border-black bg-white p-6 sm:p-8 shadow-[8px_8px_0_0_rgba(0,0,0,1)]">
                  ...
                  <div className="flex justify-between items-center py-4"><span className="text-[10px] font-black uppercase tracking-widest text-[#FF00FF]">$AUTH</span><span className="font-black flex items-center gap-1 text-[#39FF14]"><CheckCircle2 className="w-4 h-4" /> ผ่าน_</span></div>
                </div>
                <div className="flex gap-3 mt-4">
                  ...
                  <button type="submit" disabled={isSubmitting} className="flex-1 py-4 border-[4px] border-black bg-[#FF00FF] text-white font-black uppercase tracking-widest hover:bg-[#39FF14] hover:text-black transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-[10px] shadow-[4px_4px_0_0_rgba(0,0,0,1)] active:shadow-none active:translate-x-0.5 active:translate-y-0.5">
                    {isSubmitting ? 'กำลังส่ง_' : <><Send className="w-5 h-5" strokeWidth={3} /> ส่งข้อมูล_</>}
                  </button>
                </div>
              </div>
            )}
